...
def mock_github_client_error(mocker):
    ...
    mocker.patch(
        "app.core.github.GithubClient._authenticate",
...
def test_runs_endpoint_invalid_payload(test_client_openai):
    ...
    response = test_client_openai.post("/api/v1/runs", json={"invalid": "payload"})
    assert response.status_code == 422  # FastAPI validation error
    assert response.json()["detail"] == "Invalid input format"


def test_runs_endpoint_invalid_github_details(
    test_client_openai, mock_github_client_error
):
...
def test_runs_endpoint_github_error(
    mock_download_repo, test_client_openai, test_payload, mock_github_client
):
    """Test the runs endpoint when GitHub operations fail."""
    mock_download_repo.side_effect = Exception("GitHub error")

    response = test_client_openai.post("/api/v1/runs", json=test_payload)
...
def test_runs_endpoint_analysis_error(
    mock_analyze,
    mock_download_repo,
    test_client_openai,
    test_payload,
...
@patch("app.core.github.GithubClient.download_repo_zip")
@patch("app.graph.graph.StaticAnalyzerWorkflow.analyze")
def test_runs_endpoint_azure_configuration(
    mock_analyze,
    mock_download_repo,
    test_client_azure,
    test_payload,
    ...
    with patch(
        "app.core.github.GithubClient.download_repo_zip"
    ) as mock_download_repo, patch(
        "app.graph.graph.StaticAnalyzerWorkflow.analyze"
    ) as mock_analyze:
        ...
        response = test_client_azure.post("/api/v1/runs", json=test_payload)
...
@patch("app.core.github.GithubClient.download_repo_zip")
@patch("app.graph.graph.StaticAnalyzerWorkflow.analyze")
def test_runs_endpoint_openai_configuration(
    mock_analyze,
    mock_download_repo,
    test_client_openai,
    test_payload,
