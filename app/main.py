...
from core.utils import check_required_binaries, load_environment_variables
...
class Log422Middleware(BaseHTTPMiddleware):
    """
    Middleware for logging details of 422 Unprocessable Entity errors.
    Log422Middleware intercepts HTTP responses generated by the application.
    When a response has a 422 status code, this middleware attempts to extract
    the request body and logs key information including the request URL,
    HTTP method, headers, and query parameters. This logging is intended to help
    diagnose validation errors within the API.

    Methods:
        dispatch(request: Request, call_next):
            An asynchronous method that handles the incoming request by passing it
            to the next middleware or route handler. If the resulting response has a
            422 status code, the method logs detailed information about the original
            request, including a fallback message if the request body cannot be parsed.
    """
